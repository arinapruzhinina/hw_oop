...
class Triangle:
    ...
    def perimetr(self) -> float:
        return round((self.first_side + self.second_side + self.third_side), 2)

    def area(self) -> float:
        p = (self.first_side + self.second_side + self.third_side) / 2
        return round((p * (p - self.first_side) * (p - self.second_side) * (p - self.third_side)) ** 0.5, 2)
    ...
    def is_valid(self) -> bool:
        ...
        for side in all_sides:
            ...
            if side <= 0:
                return False
        return all_sides[2] < all_sides[0] + all_sides[1] # треугольник существует если сумма двух любых сторон больше третьей
